...
---Returns a modified version of `deposit` that has the same data that `source` has. Data present in `deposit` but not `source` is unmodified. Is mostly used with `deposit` as an empty table and `source` the default save data to overrite existing data with the default data.
---@param deposit table
---@param source table
...
-- Recursively validates if a table can be encoded into valid JSON. Returns 0 if it can be encoded, 1 if it can but has a warning, and 2 if item cannot. If 1 or 2, it will also return a message.
function POR.SaveCompiler.Utility.ValidateForJson(tab)
	...
	for index, value in pairs(tab) do
		...
		-- check for NaN and infinite values; http://lua-users.org/wiki/InfAndNanComparisons
...
-- Returns the dimension ID the player is currently in. 0: Normal Dimension; 1: Secondary dimension, used by Downpour mirror dimension and Mines escape sequence; 2: Death Certificate dimension
...
function POR.SaveCompiler.Save()
	...
	-- Create backup; pcall deep copies the data to prevent errors from being thrown; errors thrown in unload callback crash isaac
...
function POR.SaveCompiler.Utility.GetPickupIndex(pickup)
	...
	if myosotisCheck or movingBoxCheck then
		--Trick code to pulling previous floor's data only if initseed matches. Even with dupe initseeds pickups spawning, it'll go through and init data for each one
...
---Gets the pickup's persistent data for the floor to keep track of it outside rooms. Also checks if was stored inside the boss or treasure room save data used for the Ascent. You won't use this yourself as the pickup's persistent data is immediately nulled once the pickup in the room is loaded in. Use `GetFloorSave` instead.
---@param pickup EntityPickup
---@return table?, string
...
local function onEntityInit(_, ent)
	...
	if player and isLazB[player:GetPlayerType()] and not newGame and player.FrameCount == 0 then
		...
		if not tLazInitPlayer then
			...
			initLaz = true
		--Uses EntityPlayer directly instead of EntityPtr as .Ref gets nil'd when adding then removing Birthright; This data is cleared when one of them dies anyways
...
local function postEntityRemove(_, ent)
	...
	if (game:IsPaused() and game:GetRoom():GetFrameCount() == 0) or (ent.Type == EntityType.ENTITY_PICKUP and movingBoxCheck) then
		--Although entities are removed from the previous room and this happens before POST_NEW_ROOM... Some data from the new room is already loaded, such as frame count and listindex.
		if currentListIndex ~= game:GetLevel():GetCurrentRoomDesc().ListIndex then
			checkLastIndex = true
		end
...
local function postNewRoom()
	...
	--For knowing what the last room was after travelling down a floor in the same room; Doesn't matter if its not boss/treasure
...
---Returns a save that lasts the duration of the current floor, but data is separated per-room. **NOTE:** If your data is a pickup, use POR.SaveCompiler.GetRerollPickupSave/NoRerollPickupSave instead
...
---Attempts to return a save that lasts the duration of the current floor, but data is separated per-room. **NOTE:** If your data is a pickup, use POR.SaveCompiler.TryGetRerollPickupSave/TryGetNoRerollPickupSave instead
---@param ent? Entity | integer @If an entity is provided, returns an entity specific save within the room save, which is a floor-lasting save that has unique data per-room. If a grid index is provided, returns a grid index specific save. Otherwise, returns arbitrary data in the save not attached to an entity.
---@param noHourglass? false|boolean @If true, it'll look in a separate game save that is not affected by the Glowing Hourglass.
---@param listIndex? integer @Returns data for the provided `listIndex` instead of the index of the current room.
---@param allowSoulSave? boolean @If true, if the `ent` is The Soul attached to The Forgotten, will return a differently indexed save, as opposed to a shared save between the two.
...
---Returns the save table used for Glowing Hourglass backups. It holds two copies, indexed by 0 and 1 for the different hourglass slots provided by the REPENTOGON callbacks. If not using REPENTOGON, will only ever populate slot 0 instead
